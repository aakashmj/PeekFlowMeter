...
public class UsbECGView extends SurfaceView implements SurfaceHolder.Callback {
	...
	private UsbCanvasTimer _updateListTask; // Sangeeta changes
	 ...
	 * @param context
	 */
	public UsbECGView(Context context, MainActivity liveecg) {
		super(context);
	...
	protected void DrawECG(int[] _ECGData) {
		...
		try {
			...
			if (m_selectedmodechanged) {
				...
				offsetArray[0] = plottingheight / 14;
	...
	public void DrawNibp() {
		...
		try {
			...
			offScrCanvas.drawRect(0, 0, plottingwidth, plottingheight,
					scrPaint);
	...
	private void StartTimer() {
		...
		if (_updateListTask == null || _PlotTimer == null) {
			_updateListTask = new UsbCanvasTimer(getHolder(), this);
